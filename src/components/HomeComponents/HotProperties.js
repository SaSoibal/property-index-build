...
import { hotPropertyAction } from "../../pages/DefaultPages/redux/hot-property/hotProperty.actions";
...
function HotProperties() {
    ...
    useEffect(() => {
        dispatch(hotPropertyAction(activeTab));
        count = count + 1;
    }, [count, activeTab]);

    const property = useSelector(
        (state) => state.hotPropertyState
    );
    const propertyData = property?.hotPropertyData?.data?.website;
