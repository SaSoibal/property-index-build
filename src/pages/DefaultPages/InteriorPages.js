import React, {useContext, useEffect} from 'react'
import DefaultLayout from "../../components/layout/defaultlayout/DefaultLayout"
import BannerImg from "../../asstes/images/details01.jpg"
import { Container, Row, Col } from 'react-bootstrap'
import {Link} from 'react-router-dom'
import LanguageContext from "../../context/LanguageProvider";
import {useDispatch, useSelector} from "react-redux";
import {interiorAction} from "./redux/interior/interior.actions";
import {Skeleton} from "antd";
import {ENVIRONMENT} from "../../config/environment/environment";
import {t} from "i18next";
function InteriorPages() {
  const { language } = useContext(LanguageContext);
  let count = 0;
  const dispatch = useDispatch();
  useEffect(() => {
    if(count == 0)
        { setTimeout(()=>{
          dispatch(interiorAction());
          count = count + 1;
        },500)
    }
  }, [count, language]);
  const interior = useSelector(
      (state) => state.interiorState
  );
  const loading = interior?.loading;
  const interiors = interior?.interiorData?.data?.interior;
  return (
    <div>
        <DefaultLayout>
        <div className='blog_deatils_area'>
          <img src={BannerImg} alt="" />
           <h3> {t('interior-service')} </h3>
        </div>
        <div className='buy_sell_guide_area'>
          <Container>
            <Row>
              <Col sm={12} lg={12}>
                <div className='realHdr__text'>
                  <h3>{t('choose-property-index-interior-service')}</h3>
                  <p> {t('what-are-you-interested-in')} </p>
                </div>
              </Col>
              {loading? <><Skeleton /></> : <>
                  {interiors?.map((item, i) =>
                      <div key={i} className="col-sm-6 col-lg-6">
                        <Link to={`/interior-details/${item.slug}`}>
                          <div className="popular-places">
                            <div className="popular-places-inner">
                              <div className="popular-places-overflow">
                                <div className="popular-places-photo">
                                  <Link to={`/interior-details/${item.slug}`}>
                                    <img className="img-fluid w-100" src={ENVIRONMENT.FILE_URL+item.thumbnail} alt={language=='en'?item.title_en:item.title_bn} />
                                  </Link>
                                </div>
                                <div className="info">
                                  <h3>
                                    <Link to="/interior-details/slug"> {language=='en'?item.title_en:item.title_bn} </Link>
                                  </h3>
                                </div>
                              </div>
                            </div>
                          </div>
                        </Link>
                      </div>
                  )}
              </>}
            </Row>
          </Container>
        </div>
      </DefaultLayout>
    </div>
  )
}

export default InteriorPages
